...
#[derive(Serialize, Deserialize)]
struct HeartCommentJson {
    id: i32
}

#[post("/heart-comment", format = "json", data = "<input>")]
fn heart_comment(input: Json<HeartCommentJson>, conn: CoolDb) -> Json<Comment> {
    Json(Comment::heart(input.id, &conn.0))
}

pub fn fuel(rocket: Rocket) -> Rocket {
    rocket.mount("/", routes![create_post, delete_post, create_comment, delete_comment, heart_comment])
}